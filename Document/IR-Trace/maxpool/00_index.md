# MaxPool + BN + LIF (融合) —— IR 逐 Pass 跟踪索引

## kernel 信息

- **函数名**：`triton_poi_fused__native_batch_norm_legit_no_training__to_copy_add_convolution_div_ge_max_pool2d_with_indices_select_sub_view_9`
- **说明**：融合了 MaxPool2d 下采样、BatchNorm 与 LIF 脉冲神经元的逐元素 kernel。Inductor pointwise kernel。
- **编译选项**：`num_warps=8`，target `cuda:120`（sm_120, Blackwell）
...
## 完整 Pass 流水线（73 个 Pass，其中 22 个改变了 IR）
