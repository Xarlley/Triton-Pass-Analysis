# 矩阵乘法 / 全连接 (addmm) —— IR 逐 Pass 跟踪索引

## kernel 信息

- **函数名**：`triton_tem_fused__to_copy_add_addmm_div_ge_mul_rsub_select_sub_t_view_46`
- **说明**：VGG16 分类器全连接层（addmm = X·Wᵀ+b），融合 LIF 脉冲神经元。由 Inductor 的 Triton 矩阵乘法模板（max_autotune）生成，含 K 维归约循环。
- **编译选项**：`num_warps=2, num_stages=5`，target `cuda:120`（sm_120, Blackwell）
...
## 完整 Pass 流水线（73 个 Pass，其中 29 个改变了 IR）
