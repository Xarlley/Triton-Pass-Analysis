# BatchNorm + LIF 脉冲神经元 (逐元素) —— IR 逐 Pass 跟踪索引

## kernel 信息

- **函数名**：`triton_poi_fused__native_batch_norm_legit_no_training__to_copy_add_convolution_div_ge_mul_rsub_select_sub_view_4`
- **说明**：融合了 BatchNorm 推理归一化与 LIF 脉冲神经元（充电 / Heaviside 发放 / ATan 替代梯度）的逐元素 kernel。Inductor pointwise kernel。
- **编译选项**：`num_warps=4`，target `cuda:120`（sm_120, Blackwell）
...
## 完整 Pass 流水线（73 个 Pass，其中 23 个改变了 IR）
...
| 18 | TritonGPUAssignLatencies | `tritongpu-assign-latencies` |  | — |
| 19 | TritonGPUScheduleLoops | `tritongpu-schedule-loops` |  | — |
...
| 35 | TritonGPUHoistTMEMAlloc | `tritongpu-hoist-tmem-alloc` |  | — |
...
| 38 | TritonLoopAwareCSE | `triton-loop-aware-cse` |  | — |
...
| 57 | SCFToControlFlowPass | `convert-scf-to-cf` |  | — |
