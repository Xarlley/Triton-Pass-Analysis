...
| **必要性** | 弱。稀疏度是运行期属性、编译期看不到；纯时间拆分（切 kernel）增益有限（Inductor 已按 kernel autotune）。真正有价值的「利用稀疏度」是运行期零值检测（§2.2），与时间拆分正交、不以它为前提。 |
...
4. **真正的收益来源是利用脉冲稀疏度（§2.2 基于块的零值检测）**——它是运行期优化、
   对所有时间步一体适用、不依赖时间拆分，且不受「时间步在 IR 中已不可辨识」的阻碍。

> 一句话：现有 `MyNoOpPass`（TritonGPU C++ Pass）路线对 §2.1 的时间/空间拆分都处在
> **错误的层级**。要么上移到 Inductor 图层，要么转向 §2.2 的运行期零值检测。
