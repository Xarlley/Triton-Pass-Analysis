# SNN → SpikingJelly → IR → GPU：五个预训练模型的真实下降流程

本目录用**真实运行截获**的代码与各级 IR，说明 5 个 SNN 预训练模型，如何从 SpikingJelly 框架开始、经过 `torch.compile`/Inductor/Triton 的逐层降级，最终落到 A100 GPU 上执行。
...
| [`ottt_vgg11_cifar10/`](./ottt_vgg11_cifar10/) | OTTT-A VGG-11-WS (T=6, CIFAR-10) | 脉冲 **CNN**（权重标准化） | 多步 **LIF**（τ=2, **软重置**） | 3×3 WS 卷积 |
| [`ms_resnet104_imagenet/`](./ms_resnet104_imagenet/) | MS-ResNet-104 (T=6) | 脉冲 **CNN**（膜电位残差+TDBN） | 多步 **LIF**（τ=4/3, 硬重置） | 3×3 卷积 |

> 后两个（OTTT、MS-ResNet）的原仓库**不用** spikingjelly 多步节点（OTTT 是在线单步 `OnlineLIFNode`，MS-ResNet 是纯 torch 时间循环 `mem_update`），因此是真正的**重写**为 spikingjelly `activation_based` 多步 LIF + Triton 后端，并逐样本/逐位验证与原模型一致（见各自 README）。
...
| OTTT-A VGG-11-WS | [`ottt_vgg11_cifar10/scripts/`](./ottt_vgg11_cifar10/scripts/) | ✅ | ✅ | — |
| MS-ResNet-104 | [`ms_resnet104_imagenet/scripts/`](./ms_resnet104_imagenet/scripts/) | ✅ | ✅ | — |

- 每个 `scripts/` 含模型专属脚本 + `_common/`（共享的 `sj_compat.py` / `snn_eval_lib*.py` /
  `snn2_lib.py` / `timm_compat.py` / `capture_ir*.py` / `verify_equiv.py` 等，各副本内容相同），并有 `README.md` 逐文件说明。
- OTTT/MS-ResNet 的专属脚本：`ottt_vgg_triton.py` / `run_ottt.py`、`ms_resnet_triton.py` / `run_msresnet.py`；
  共享 `snn2_lib.py`（CIFAR-10 lossless eval + 模型无关 profiler + inductor-triton 配置）、`verify_equiv.py`（重写==原模型校验）。
...
## 结果（验证集子集；profile = 单 batch CUDA kernel 时间占比）

| 模型 | 数据集 | top-1（triton 神经元, eager） | top-1（整网 triton, bf16+compile） | 论文 | Triton 占比 | cublas | cudnn |
|------|------|------|------|------|------|------|------|
| SEW-ResNet-34 | ImageNet | 67.75% | 67.10% | 67.04% | **100%** | 0 | 0 |
| Spikingformer-8-768 | ImageNet | 75.95% | 76.55% | 75.85% | **100%** | 0 | 0 |
| Meta-SpikeFormer-55M | ImageNet | 79.90% | 79.85% | ~80.0% | 64% | **0** | **0** |
| OTTT-A VGG-11-WS | CIFAR-10 | 93.60% | 93.70% | 93.52% | **100%** | **0** | **0** |
| MS-ResNet-104 | ImageNet | 74.20% | 74.00% | 76.02%¹ | 98.7% | **0** | **0** |

¹ 论文为全 5 万张 val；此处为前 2000 张子集。
- 神经元 Triton 后端与 torch 后端逐位一致（OTTT argmax 全一致；MS-ResNet/前三者 maxdiff=0），精度全部复现。
- **五模型 cublas/cudnn 均为 0。** 非-100%-Triton 的残留都不是 cudnn/cublas：Meta-SpikeFormer 36% = ATEN **depthwise 卷积**（7×7 SepConv，无 Triton 模板）；MS-ResNet 1.3% = ATEN **`adaptive_avg_pool3d`**（无自适应 3D 池化 Triton 模板）。
- 两类重置在 IR 上可见：**软重置**（OTTT：`V=H−S·v_th`）vs **硬重置**（SEW/MS-ResNet：`V=(1−S)·H`）。OTTT 还把**权重标准化**(mean/var/gain)编译进图并与卷积融成 Triton。
